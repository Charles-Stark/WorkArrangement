  ...
  <div>
    <v-card :color="$vuetify.theme.dark === false ? 'white' : '#303030'" flat>
      ...
      <v-stepper v-model="e1" vertical>
        <v-stepper-step :complete="e1 > 1" step="1" class="title">
          基本信息
        </v-stepper-step>
            ...
            <v-card-text>
              <v-form ref="form1">
                <v-row>
                  ...
                  <v-col cols="6">
                    <v-text-field type="number" label="排班时长" v-model="arrangeConfig.basic.last"
                      @blur="arrangeConfig.basic.last <= 0 ? arrangeConfig.basic.last = 30 : arrangeConfig.basic.last = arrangeConfig.basic.last"></v-text-field>
                  </v-col>
                </v-row>
              </v-form>
            </v-card-text>
        ...
        <v-stepper-step :complete="e1 > 1" step="2" class="title">
          准备工作
        </v-stepper-step>
                ...
                <v-row>
                  ...
                  <v-col cols="3">
                    <v-text-field required v-model="arrangeConfig.pre.workLoad" dense label="人均工作量" type="number"
                      :rules="noneEmptyRule" @blur="arrangeConfig.pre.workLoad<=0? arrangeConfig.pre.workLoad=100: arrangeConfig.pre.workLoad= arrangeConfig.pre.workLoad" class="mt-3"></v-text-field>
                  </v-col>
                  <v-col cols="2">
        ...
        <v-stepper-step :complete="e1 > 2" step="3" class="title">
          排班安排
        </v-stepper-step>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-row>
                ...
                <v-col cols="3">
                  <v-text-field required v-model="arrangeConfig.in.attendance" type="number" dense suffix="人" class="mt-3"
                    @blur="arrangeConfig.in.attendance<=0? arrangeConfig.in.attendance=1: arrangeConfig.in.attendance= arrangeConfig.in.attendance" :rules="noneEmptyRule"></v-text-field>
                </v-col>
              </v-row>
            </v-card-text>
            <v-card-title>
                ...
                <v-row>
                  ...
                  <v-col cols="3">
                    <v-text-field required v-model="arrangeConfig.in.num" dense label="单个员工服务人数" type="number"
                      :rules="noneEmptyRule"  class="mt-3" @blur="arrangeConfig.in.num<=0? arrangeConfig.in.num=3.8: arrangeConfig.in.num= arrangeConfig.in.num"></v-text-field>
                  </v-col>
                  <v-col cols="3">
        ...
        <v-stepper-step :complete="e1 > 3" step="4" class="title">
          收尾工作
        </v-stepper-step>
                ...
                <v-row>
                  ...
                  <v-col cols="3">
                    <v-text-field required v-model="arrangeConfig.post.time" type="number" dense suffix="h" class="mt-3"
                      @blur="arrangeConfig.post.time<=0? arrangeConfig.post.time=2: arrangeConfig.post.time= arrangeConfig.post.time" :rules="noneEmptyRule"></v-text-field>
                  </v-col>
                </v-row>
                <v-row>
                  <v-col cols="12">
                  ...
                  <v-col cols="2">
                    <v-text-field required v-model="arrangeConfig.post.workLoad" dense label="人均工作量" type="number"
                      :rules="noneEmptyRule" @blur="arrangeConfig.post.workLoad <=0? arrangeConfig.post.workLoad =80: arrangeConfig.post.workLoad = arrangeConfig.post.workLoad" class="mt-3"></v-text-field>
                  </v-col>
                  ...
                  <v-col cols="2">
                    <v-text-field required v-model="arrangeConfig.post.offset" dense label="偏移量" type="number"
                      :rules="noneEmptyRule" @blur="arrangeConfig.post.offset <= 0 ? arrangeConfig.post.offset = 1 : arrangeConfig.post.offset = arrangeConfig.post.offset" class="mt-3"></v-text-field>
                  </v-col>
                  <v-col cols="3">
