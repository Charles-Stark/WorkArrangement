<template>
  <div
    v-if="
      ((branches.length !== 0 && $store.state.isManager) ||
  ...
  >
    ...
    >
      ...
      ></v-select>
      <span v-if="!$store.state.isManager" class="text-h6 ml-3">{{
        shopName
      }}</span>
      <v-spacer></v-spacer>
      ...
      <v-sheet>
        <v-toolbar flat color="transparent">
          ...
          <v-dialog
            ...
            v-model="newAbsc"
            v-if="
              !this.$store.state.isManager & !this.$store.state.isShopManager
            "
          >
            <template v-slot:activator="{ on, attrs }">
              <v-btn
          ...
          <v-btn-toggle v-model="type" dense mandatory color="primary">
            ...
            <v-btn value="day" v-else> 日 </v-btn>
          </v-btn-toggle>
        </v-toolbar>
      </v-sheet>
      <v-sheet>
        ...
        >
          <v-card min-width="350px" flat>
            <v-toolbar :color="selectedEvent.color" dark>
              ...
              <v-toolbar-title
                class="ml-2"
                v-if="type === 'week' && !selectedEvent.single"
                >{{
                  String(
                    new Date(selectedEvent.end - 1800000).getHours()
                  ).padStart(2, "0") +
                  ":" +
                  String(
                    new Date(selectedEvent.end - 1800000).getMinutes()
                  ).padStart(2, "0") +
                  " - " +
                  String(new Date(selectedEvent.end).getHours()).padStart(
                    2,
                    "0"
                  ) +
                  ":" +
                  String(new Date(selectedEvent.end).getMinutes()).padStart(
                    2,
                    "0"
                  )
                }}</v-toolbar-title
              ...
              <v-toolbar-title
                class="ml-1"
                v-if="selectedEvent.single === true"
                >{{
                  selectedEvent.showStart + " - " + selectedEvent.showEnd
                }}</v-toolbar-title
              >
              ...
              <v-btn
                icon
                v-if="selectedEvent.single === true || type !== 'week'"
                @click="
                  selectedEmployee = 0
                  dialog = true
                "
            ...
            <div v-if="ready2">
              ...
              <div v-if="type !== 'week' || selectedEvent.single === true">
                <v-list-item-title
                  class="ml-3 grey--text text-subtitle"
                  v-if="search2 === null"
                  >推荐员工</v-list-item-title
                >
                <v-list-item
                  @click="
                    ...
                    dialog = true
                  "
                  v-for="s of filteredStaff.slice(0, 3)"
                  :key="s.id"
                >
                  <v-list-item-avatar>
              ...
              <v-virtual-scroll
                ...
                bench="10"
                :items="
                  selectedEvent.single === true
                    ? filteredStaff.slice(3)
              ...
              >
                <template v-slot:default="{ item, index }">
                  <v-list-item
                    @click="
                      selectedEvent.single === true
                        ? ((selectedEmployee = item.id), (dialog = true))
                        : openSelected(item, index)
                    "
                  >
                    <v-list-item-avatar>
  ...
  <v-container v-else-if="!ready1" style="height: 80vh">
    <v-row class="fill-height" align-content="center" justify="center">
      <v-col class="text-h5 text-center" cols="12"> 正在加载排班表 </v-col>
      <v-col cols="6">
        <v-progress-linear
          color="primary accent-4"
          indeterminate
  ...
  <v-container v-else style="height: 80vh">
    <v-row class="fill-height" align-content="center" justify="center">
      <v-col class="text-h5 text-center" cols="12"> 没有店铺信息 </v-col>
      <v-col cols="12" class="text-center">
        <v-btn outlined color="primary" to="/controlpanel/branches"
          >点击跳转到分店页<v-icon>mdi-arrow-right</v-icon></v-btn
        >
      </v-col>
    </v-row>
  </v-container>
...
export default {
    ...
    scheduleType() {
      const rawEvents = this.rawEvents.filter((r) => {
        return r.detail.some((e) => {
          if (e !== undefined)
            return (
              e.position.indexOf(this.search1 || "") !== -1 ||
              e.username.indexOf(this.search1 || "") !== -1 ||
              e.uid.indexOf(this.search1 || "") !== -1
            )
        })
      })
      rawEvents.forEach((r) => {
  ...
  methods: {
    GetExcel() {
       ...
       * */
      for (let i = 0; i < this.something_data.weeks.length; i++) {
        //周
        for (let j = 0; j < this.something_data.weeks[0].data.length; j++) {
          //天
          for (
            let x = 0;
            x < this.something_data.weeks[0].data[0].length;
            x++
          ) {
            //时
            /*console.log("i="+i)
            ...
            let weeksData = {
              ...
              beginTime: formatDate(
                this.something_data.weeks[i].data[j][x].beginTime
              ),
                ...
                for (let event of day) {
                  if (event !== null) {
                    ...
                    event.employees.forEach((employee) => {
                      detail.push(
                        this.staff.find((item) => item.id === employee)
                      )
                    })
                    ...
                    let endTime
                    if (
                      beginTime.getMinutes() === 30 &&
                      beginTime.getHours() === 23
                    ) {
    ...
    async openSelected(item) {
      ...
      for (let i = 0; i < selectedEmployee.start.length; i++) {
        if (
          this.selectedEvent.start >= selectedEmployee.start[i] &&
          this.selectedEvent.end <= selectedEmployee.end[i]
        ) {
